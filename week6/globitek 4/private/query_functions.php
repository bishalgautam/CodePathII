<?php
  ...
  function find_failed_login($username){
    ...
    $sql .= "LIMIT 1;";
    $failed_attempt = db_query($db, $sql);
  ...
  function validate_user($user, $errors=array()) {
    ...
    }else if(!has_length($user['password'], array('min' => 12)) || !has_length($user['cpassword'],array('min' => 12))){
      $errors[] = "Password cannnot be less than 12 characters";
    }else if (!is_valid_password($user['password']) || !is_valid_password($user['cpassword'])){
      $errors[] = "Password must have at least one uppercase letter, lowercase letter, number, and symbol.";
    }
    return $errors;
  }
  ...
  function insert_user($user) {
    ...
    $hashed_password = password_hash(db_escape($db, $user['password']), PASSWORD_BCRYPT);
